...
| Folder | Contents |
|---|---|
| `sql/` | the full pipeline, raw CSV → recommendation |
| `tableau/` | flat exports feeding the dashboard |

## Run order

| File | Builds | Expected result |
|---|---|---|
...
## Why expected row counts are written into every script

A count that moves for a reason you can't name is the most reliable bug signal
in SQL. Three defects in v1 were found exactly this way — including one that
cost a single row out of 99,224, caused by MySQL treating a backslash in a
Portuguese review comment as an escape character.

## Fixed relative to v1

- **Reviews pre-aggregated** to one row per order before joining.
  `order_reviews` has no unique key (99,224 rows, 98,410 distinct `review_id`),
  so joining it directly multiplies item rows.
- **NULL-category exclusion stated explicitly** in the `WHERE` clause instead of
  happening invisibly through a join on a nullable column.
- **Category benchmark restricted to delivered orders**, matching the population
  compared against it.
...
- **Monthly trend** — discount rate flat at 68–72% across 19 months while volume
